...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defun %window-property-case (window test cases)
    (when cases
      `(if (funcall ,test ,window ,(caar cases))
           (progn ,@(cdar cases))
           ,(%window-property-case window test (cdr cases))))))

(defmacro window-property-case ((window test) &rest cases)
  (let ((win (gensym))
        (tst (gensym)))
    `(let ((,win ,window)
           (,tst ,test))
       ,(%window-property-case win tst cases))))
...
(defun print-key-structure (key stream)
