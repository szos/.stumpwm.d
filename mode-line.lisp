...
(defun only-active-window-title (ml)
  (declare (ignore ml))
  (window-title (group-current-window (current-group))))

(add-screen-mode-line-formatter #\a 'only-active-window-title)
...
(defun open-hwmon-files (f1 f2 f3)
  (with-open-files ((t1 f1)
                    (t2 f2)
                    (t3 f3)) 
...
;; (add-screen-mode-line-formatter #\G 'fmt-visible-group-list)
;; (defun fmt-visible-group-list (ml))

(setf *screen-mode-line-format*
      (list "^[^68[^58%g^68]%W^>^6^BCPU:%U MEM:%M% BAT:%B %h^b%T"))
